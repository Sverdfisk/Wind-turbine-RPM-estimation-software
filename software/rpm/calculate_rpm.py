...
frame_time = 1/fps
pixel_radius = 130
adjusted_radius = 0.8*pixel_radius #If we average out all vectors, they will end up at approx. 3/4 the way to the edge of the blade.
...
for block in data:
    ...
    angular_velocity = mag_avg/(adjusted_radius*frame_time)
    frequency = angular_velocity/(3.1415) # should be divided by 2*pi, but this is removed for easier calculation.

    rpm = round(frequency*30, 3) #See comment above, rpm is found with *30 here because of earlier division.
    rpms.append(rpm)
